...
function buildMutationContextDocument(
  ...
  // Cold-start race: on the first Hocuspocus connection, the Yjs marks
  // preservation cache may not yet be populated, leaving mutationBase.marks = {}.
  // In that case, fall back to the canonical doc marks (which already contain
  // any pending marks from mergePreservedActionMarks via the DB row).
  const baseMarks = Object.keys(mutationBase.marks).length > 0
    ? mutationBase.marks
    : (parseMarksPayload(doc.marks) ?? {});
  return {
    ...doc,
    markdown: mutationBase.markdown,
    marks: JSON.stringify(baseMarks),
