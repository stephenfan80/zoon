...
// 第一次用 Zoon 的人打开新文档，这里只讲真实编辑页机制：
// Agent 用 HTTP 直接读写；评论/建议是主动选择，不是默认拦截。
const DEFAULT_MARKDOWN = `# 欢迎用 Zoon 写文档

Zoon 是 agents 和 humans 共写的在线文档空间。你写的字、Agent 写的字都会带来源；左边那条彩色边栏会告诉你每一段是谁写的。

## 协作模型分三层

1. **评论 = 任务 / 讨论**

   想让 Agent 先理解需求、解释判断、补充信息，或讨论某段怎么改，就留评论。评论不会改正文，它更像任务单和讨论串。

2. **建议 = 待确认改动**

   想让 Agent 先给替换方案，就让它用建议。建议会贴在原文旁边，等你确认；在你接受或拒绝前，正文不算真正被改。

3. **正文 = 真正改动**

   让 Agent 直接补一段、加一节、整理素材，或你接受某条建议，才会真正改正文。每段文字都会保留作者身份，方便你审校。

## 两条协作规则

1. **一个链接就是协作入口**

   你打开链接就是编辑器；Agent 拿到同一个链接和 token，就能通过 HTTP 读取文档、写文档、发 presence、留评论。

2. **需要哪一层，就明确告诉 Agent**

   想讨论就说“留评论”；想先看方案就说“给建议”；想让它直接落稿就说“改正文”。Zoon 的控制感来自来源可见、版本可回溯、评论 / 建议 / 正文分层，而不是默认挡住 Agent 的每一次修改。
...
3. **你审校** — 看到不对的内容可以直接改、删，或让 Agent 重写；需要先讨论时就让 Agent 留评论或建议。
...
- 写一句自己的原文，再让 Agent 直接改写它；然后再试一次，让它改用 suggestion 提案
