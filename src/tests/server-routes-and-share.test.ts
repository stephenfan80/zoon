...
async function runRoutePayloadValidationTests(): Promise<void> {
  await withEphemeralApiServer(async (baseUrl) => {
    ...
    await test('D2: POST /share/markdown works without client headers', async () => {
      ...
      const payload = await response.json();
      assert(typeof payload.url === 'string' && payload.url.includes('/d/'), 'Expected url');
      assert(payload.url.includes('?token='), `Expected url to be tokenized, got ${String(payload.url)}`);
      assert(typeof payload.shareUrl === 'string' && payload.shareUrl.includes('/d/'), 'Expected shareUrl');
      assert(payload.shareUrl.includes('?token='), `Expected shareUrl to be tokenized, got ${String(payload.shareUrl)}`);
      assertEqual(payload.url, payload.tokenUrl, 'Expected top-level url to match tokenUrl');
      assertEqual(payload.shareUrl, payload.tokenUrl, 'Expected top-level shareUrl to match tokenUrl');
      assert(typeof payload.viewUrl === 'string' && !payload.viewUrl.includes('?token='), 'Expected viewUrl to remain the clean human URL');
    ...
    await test('D2: POST /share/markdown creates share link from JSON markdown payload', async () => {
      ...
      assert(typeof payload.slug === 'string' && payload.slug.length > 0, 'Expected slug');
      assert(typeof payload.url === 'string' && payload.url.startsWith(`${baseUrl}/d/`), 'Expected absolute tokenized URL');
      assert(payload.url.includes('?token='), `Expected url to include token, got ${String(payload.url)}`);
      assert(typeof payload.shareUrl === 'string' && payload.shareUrl.startsWith(`${baseUrl}/d/`), 'Expected absolute share URL');
      assert(payload.shareUrl.includes('?token='), `Expected shareUrl to include token, got ${String(payload.shareUrl)}`);
      assertEqual(payload.url, payload.tokenUrl, 'Expected top-level url to match tokenUrl');
      assertEqual(payload.shareUrl, payload.tokenUrl, 'Expected top-level shareUrl to match tokenUrl');
      assert(typeof payload.viewPath === 'string' && payload.viewPath.startsWith('/d/'), 'Expected viewPath to keep the relative clean URL');
