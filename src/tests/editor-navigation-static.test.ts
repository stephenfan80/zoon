...
assert(!nav.includes("this.outlineShell.addEventListener('pointerenter'"), 'Desktop outline should no longer depend on hover to open');
assert(!nav.includes("this.outlineShell.addEventListener('pointerleave'"), 'Desktop outline should no longer close after pointer leaves');
assert(!nav.includes('OUTLINE_CLOSE_DELAY_MS'), 'Persistent desktop outline should not use a hover close delay');
...
assert(nav.includes("document.body.classList.toggle('editor-outline-visible', showOutline);"), 'Persistent outline should reserve document-side space while visible');
assert(nav.includes("document.body.classList.remove('editor-outline-visible');"), 'Persistent outline reserve should be cleaned up on destroy');
assert(nav.includes("headerLabel.textContent = '目录';"), 'Persistent outline panel should label the table of contents');
...
assert(nav.includes('this.outlinePanel.hidden = !showOutline;'), 'Outline panel should be visible whenever the outline is available');
assert(!nav.includes('this.closeOutlinePanel();'), 'Outline item clicks should not close a floating panel');
...
assert(html.includes('--outline-aside-width: 184px;'), 'Desktop outline should reserve a readable right-side aside width');
assert(html.includes('--editor-outline-reserve: 0px;'), 'Outline reserve should be opt-in until a document has enough headings');
assert(html.includes('body.editor-outline-visible'), 'Document layout should reserve space when the right outline is visible');
...
assert(html.includes('display: none;'), 'Desktop outline toggle should not appear as a separate floating layer');
assert(html.includes('position: static;'), 'Desktop outline panel should live in the right aside instead of a fixed popover');
assert(html.includes('box-shadow: none;'), 'Persistent outline panel should not look like a floating popover');
