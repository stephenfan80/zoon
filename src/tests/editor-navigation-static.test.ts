...
assert(html.includes('--outline-panel-left: calc(var(--provenance-gutter-left) + var(--provenance-bar-width) + 8px);'), 'Outline panel should sit beside the left navigation without covering the provenance bar');
assert(html.includes('top: calc(50vh - 161px);'), 'Outline handle should avoid transforms so fixed child panels stay viewport anchored');
assert(html.includes('top: 50vh;'), 'Outline panel should align vertically with the centered outline handle');
assert(html.includes('transform: translateY(-50%);'), 'Outline panel should center itself beside the outline handle');
...
assert(!html.includes('top: 32px;'), 'Outline panel should not float near the top like a detached page popover');
