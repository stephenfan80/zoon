...
class ProofEditorImpl implements ProofEditor {
  ...
  private async initFromShare(options?: ShareRuntimeActivationOptions): Promise<void> {
    ...
    try {
      ...
      // Set title
      document.title = doc.title ? `${doc.title} - Zoon` : 'Shared Document - Zoon';
  ...
  private createAccountMenuButton(): HTMLElement {
    ...
    btn.style.cssText = `
      display:inline-flex;align-items:center;justify-content:center;min-height:38px;min-width:48px;padding:0 14px;
      background:#f4f5f1;border:1px solid rgba(0,0,0,0.08);border-radius:20px;color:#374151;
      font-size:13px;font-weight:600;cursor:pointer;transition:background 0.15s, border-color 0.15s;
      flex-shrink:0;font-family:inherit;
    `;
    btn.onmouseenter = () => { btn.style.background = '#eceee8'; };
    btn.onmouseleave = () => { btn.style.background = '#f4f5f1'; };
    ...
    const renderSignedOutMenu = (menu: HTMLElement): void => {
      ...
      const makeInput = (name: string, labelText: string, placeholder: string, type: string = 'text'): HTMLInputElement => {
        ...
        input.autocomplete = name === 'password' ? 'current-password' : (name === 'email' ? 'email' : 'off');
        input.style.cssText = `
          width:100%;min-height:38px;border:1px solid rgba(255,255,255,0.14);border-radius:10px;
    ...
    const openMenu = async (): Promise<void> => {
      ...
      if (!menu.isConnected) return;
      if (!user) renderSignedOutMenu(menu);
      else await renderSignedInMenu(menu, user);
    };
  ...
  private openAgentHelpModal(): void {
    ...
    header.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,0.10)';
    const title = document.createElement('div');
    title.textContent = '如何使用 Agent 协作者';
    title.style.cssText = 'font-size:13px;font-weight:700;letter-spacing:0.02em';
    const close = document.createElement('button');
    close.type = 'button';
    close.textContent = '关闭';
    ...
    body.innerHTML = `
      <p style="margin:0 0 10px 0;">Agent 协作者可以与你共享相同的文档权限来建议和编辑内容。</p>
      <p style="margin:0 0 8px 0;"><strong>如何接入：</strong></p>
      <ol style="margin:0 0 10px 18px;padding:0;">
        <li>复制 Agent 邀请链接。</li>
        <li>粘贴到你的 AI 工具中（例如 ChatGPT 或 Claude）。</li>
        <li>Agent 连接后将出现在此处。</li>
      </ol>
      <p style="margin:0;color:rgba(255,255,255,0.72);">断开连接会移除该 Agent 在此文档中的实时状态，但不会撤销链接权限。</p>
    `;
  ...
  private createShareMenuButton(): HTMLElement {
    ...
    const openMenu = () => {
      ...
      addItem('复制链接', async () => this.copyLinkWithFallback(this.getCanonicalShareUrl()));
      addDivider();
      addActionItem('查看活动', () => this.openShareActivityModal());
    ...
    const openMenu = () => {
      ...
      if (agentsNow.length === 0) {
        ...
        addMenuButton('Agent 如何接入', async () => {
          ...
          return true;
        }, { subtle: true, successText: '完成' });
      } else {
        const header = document.createElement('div');
        header.textContent = `Agent 协作者 (${agentsNow.length})`;
        ...
        for (const agent of agentsNow.slice(0, 6)) {
          ...
          // AI Agent → 动物 emoji 头像
          const face = createAnimalAvatarEl(agent.id, 18);
          const dot = document.createElement('span');
